...
export const CodeTerminal = ({
  ...
  testSimpleData: TestCase[];
  testFullData: TestCase[];
  processing: boolean;
}) => {
  ...
  const formatTestMessage = (status: string, message: string) => {
    if (status === "success") {
      return (
        <pre className="p-4 text-green-500 break-words whitespace-pre-wrap">
          {message}
        </pre>
      );
    }
    ...
    if (status === "failure") {
      return (
        <pre className="p-4 text-red-500 break-words whitespace-pre-wrap">
          {message}
        </pre>
      );
    }
    // console.log(result);
    return (
      <pre className="whitespace-pre-wrap px-2 py-1 text-gray-500">
        {message}
      </pre>
    );
  };
    ...
    <div className="flex flex-col h-full w-full items-center justify-center rounded-lg border">
      ...
      <div className="h-full w-full overflow-y-auto p-1">
        <Accordion type="multiple" className="">
          {testSimpleData.length > 0 &&
            testSimpleData.map((data: TestCase, index: number) => {
              return (
                <AccordionItem key={data.testName} value={data.testName}>
                  <AccordionTrigger className="px-4">
                    <div className="inline-flex w-full items-center space-x-2 transform transition-transform duration-200 hover:translate-x-[10px] hover:scale-105">
                      <div className="">{testStatus(data?.result?.status)}</div>
          ...
          {testFullData.length > 0 &&
            testFullData.map((data: TestCase, index: number) => {
              return (
                <AccordionItem key={data.testName} value={data.testName}>
                  <AccordionTrigger className="px-4">
                    <div className="inline-flex w-full items-center space-x-2 transform transition-transform duration-200 hover:translate-x-[10px] hover:scale-105">
                      ...
                      <li className="space-y-2" key={data.testName}>
                        ...
                        <div>
                          ...
                          <Textarea
                            ...
                            style={{ resize: "none" }}
                            rows={1}
                            value={data.expectedOutput || "None"}
                            readOnly
                          />
                        </div>
                      </li>
